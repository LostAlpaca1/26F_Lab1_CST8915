...
- [Azure Cleanup (Required)](#azure-cleanup-required)
...
- **Product Service (Rust)**: Serves a product catalog with IDs, names, and prices.
...
> **Cleanup required**: Keep all Azure resources created for this lab in the dedicated `lab1-petstore-rg` resource group. When you finish the lab and save your submission materials, follow [Azure Cleanup (Required)](#azure-cleanup-required) to delete them.
...
| **3000**  | Order Service   | TCP      | 330      | Any    | Yes       | Node.js API - accepts and queues orders             |
| **3030**  | Product Service | TCP      | 340      | Any    | Yes       | Rust API - serves the product catalog               |
...
**Expected Result**: Rules for ports 8080, 3000, and 3030 are visible. Add port 15672 only if using the optional management UI.
...
2. Clone the standalone lab repository, `26F_Lab1_CST8915`:
   ```bash
   git clone <repository-url> ~/26F_Lab1_CST8915
   cd ~/26F_Lab1_CST8915
   ```
...
- Stores orders in a durable queue; this lab publishes persistent messages and waits for broker confirmation
- Critical dependency: Must be running before Order Service starts

**Installation:**
1. Navigate to the RabbitMQ directory:
   ```bash
   cd ~/26F_Lab1_CST8915/RabbitMQ
   ```
...
3. **Test the management UI** (optional but recommended):
   ...
   - Login: username `newuser`, with the password you chose in the RabbitMQ guide
...
**Expected Result**: RabbitMQ is active. If you enabled the optional management UI, it is accessible with your new account.
...
- Serves the **product catalog**: IDs, names, and prices
...
1. Navigate to the product-service directory:
   ```bash
   cd ~/26F_Lab1_CST8915/product-service
   ```

2. Follow the detailed instructions in [product-service/README.md](product-service/README.md)

3. The linked guide starts the service with `cargo run`. Keep that process running; the application does not print a listening message. From another terminal, verify it with `curl http://localhost:3030/products`.
...
- Handles **customer orders**: receiving and queuing orders
...
2. Navigate to the order-service directory:
   ```bash
   cd ~/26F_Lab1_CST8915/order-service
   ```

3. Follow the detailed instructions in [order-service/README.md](order-service/README.md)

4. The linked guide starts the service with `node index.js`. Keep that process running. Expect: `Order service is running on http://localhost:3000`.
...
2. Navigate to the store-front directory:
   ```bash
   cd ~/26F_Lab1_CST8915/store-front
   ```

3. Follow [store-front/README.md](store-front/README.md), including both public-IP API URL replacements **before its first launch**.

4. The linked guide starts the service with `npm run serve`. Keep that process running. You should see:
   ```
   App running at:
   ...
   The Network URL may show a private IP; use your Azure public IP in your laptop browser.
...
At this point, you should have **3 terminals** running the foreground services. RabbitMQ runs in the background. Use another terminal for checks as needed; the table below summarizes processes already started.

| Terminal   | Service         | Command                     | Port        | Status Check                                                  |
| ---------- | --------------- | --------------------------- | ----------- | ------------------------------------------------------------- |
| Background | RabbitMQ        | System service (background) | 5672, 15672 | `sudo systemctl status rabbitmq-server`                       |
| 1          | Product Service | `cargo run`                 | 3030        | `curl http://localhost:3030/products`                         |
| 2          | Order Service   | `node index.js`             | 3000        | Log shows "Order service is running on http://localhost:3000" |
| 3          | Store Front     | `npm run serve`             | 8080        | Open `http://<VM-IP>:8080` in browser                         |

> **Tip**: Stop a foreground service with `Ctrl + C` in its terminal. To stop RabbitMQ, use `sudo systemctl stop rabbitmq-server`.
...
- **Product Selection**: Select one product and enter a quantity
- **Order Placement**: When you place an order, it goes through the Order Service (port 3000) and into RabbitMQ

#### Test the Complete Workflow

1. **Browse Products**: You should see three product names and prices, with decorative animal images above the form
2. **Select a Product**: Select Dog Food and enter quantity 2; the total should be $39.98
3. **Place Order**: Click **Place Order** and check the success message
4. **Verify the Order Was Queued**:
   - Check the Order Service terminal - you should see log messages about the order
   - Run `sudo rabbitmqctl list_queues name durable messages` in another VM terminal; `order_queue` should be durable and its message count should increase. You can also use the optional management UI (`http://<VM-IP>:15672`).

> **Success Criteria**: Products load, selecting two Dog Food units totals $39.98, and placing an order adds the submitted payload to RabbitMQ without runtime or browser-console errors. Orders are accepted and queued; this lab includes no fulfillment consumer.
...
3. When you place an order:
   - The Order Service accepts it on port 3000
...
- [ ] **End-to-End Functionality Works**
  ...
  - Can select one product, enter a positive quantity, and see the correct total
  - Can place an order successfully
  - Order appears in RabbitMQ queue (use `rabbitmqctl` or the optional management UI)
  - Browser console has no runtime or WebSocket errors

- [ ] **Network Security Group Configured**
  - Ports 8080, 3000, and 3030 are open; port 15672 is needed only for the optional management UI
...
5. The order appears in RabbitMQ (verified with `rabbitmqctl` or the optional management UI)
...
## Azure Cleanup (Required)

**When you are done with the lab, delete all Azure resources you created for it.** Complete the following steps to avoid ongoing charges against your Azure credits.

### 1. Save Your Work

- Record and upload your demo video while the application is still running.
- Save any screenshots, notes, code changes, and other files you need from the VM to your local machine or submission repository.
- Verify that your demo video and submission files are accessible before deleting the deployment.

Deleting the resource group removes the VM and its disks, including files and queued orders stored on them. Your application URL and SSH connection will stop working.

### 2. Delete the Lab Resource Group

1. Sign in to the [Azure Portal](https://portal.azure.com/).
2. Open **Resource groups** and select **`lab1-petstore-rg`** (or the dedicated resource group name you used for this lab).
3. Check the subscription and resource list to confirm this group contains only resources for this lab. If it contains resources for other work, delete only the lab's resources individually.
4. Select **Delete resource group**.
5. Enter the resource group name when prompted and confirm deletion.
6. Wait for Azure to report that deletion has completed, then refresh **Resource groups** and verify that the lab group is gone.

Deleting the dedicated resource group removes its VM, disks, network interfaces, public IP addresses, NSG, virtual network, and any other resources in that group. See [Microsoft's resource group deletion instructions](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/manage-resource-groups-portal#delete-resource-groups).

### 3. Check for Leftover Lab Resources

Open **All resources** in the subscription you used and check for anything created for this lab outside the deleted group, including resources from failed or repeated setup attempts. Delete any remaining lab VMs, disks, snapshots, public IP addresses, network interfaces, NSGs, virtual networks, storage accounts, and Azure SSH key resources. Preserve resources that belong to other coursework or projects.

**Stopping or deallocating the VM does not complete cleanup.** Disks and some networking resources can still incur charges after deallocation. Delete all lab resources as described above. See [Azure VM states and billing](https://learn.microsoft.com/en-us/azure/virtual-machines/states-billing).
